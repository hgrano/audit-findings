...
1. Attacker calls CrossChainRouter::borrowCrossChain on Chain A.
1. Attacker waits some time so their debt increases due to interest.
1. Attacker calls CrossChainRouter::borrowCrossChain on Chain A (but can use a dust quantity for the borrow amount as it is not significant).
1. Attacker calls CrossChainRouter::repayCrossChainBorrow on Chain B, repaying only the original debt but not any interest owed.
1. Attacker calls CoreRouter::redeem on Chain A, and are able to redeem their entire collateral balance despite owing interest on Chain B.
